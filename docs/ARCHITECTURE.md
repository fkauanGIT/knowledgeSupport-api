...
| `Standard.investigationSteps` as an `@ElementCollection`, no separate JPA entity/repository | A step (hypothesis/query/verification/confirmed) has no identity or lifecycle outside its `Standard` — nobody looks up a step on its own. Same precedent as `Requester` living inside `Called`. Modeled as `InvestigationStepEmbeddable` (`@Embeddable`, `adapter/out/persistence`) in a `List` on `StandardJpaEntity` (`@ElementCollection` + `@OrderColumn` to preserve investigation order), converted by `StandardMapper`. This is a **documentation/reasoning trail**, not a new matching signal: `CalledStandardMatcher` still scores only `standardName`+`text` and still gates on `Standard.result`, unchanged. Matching is also **not gated** on having a `confirmed` step — existing registered Standards have no steps yet, and gating on it would silently stop them from matching after this ships. See `docs/LIMITATIONS.md`. |

| Read-through cache (Caffeine) with per-cache TTLs | Two hot reads dominated cost: `fetchOpenCalleds` hit Jira live on every call (and the gap-report pulls *all* open tickets), and `findAll()` of the Standards ran on every single analysis. A `CaffeineCacheManager` (`CacheConfig`) caches `openCalleds` (TTL 45s, keyed by the resolved JQL + `CalledFilter` — Jira stays the source of truth, staleness is short and bounded), `standards` (evicted on create/update/delete, so always fresh) and `documentCorpus` (see the TF-IDF row). Per-cache TTLs instead of a single global spec because the caches have different freshness needs. |
| Timeout, unified retry and a latency metric on `JiraCalledAdapter` | The `RestClient` had **no timeout** — a hung Jira connection held a Tomcat thread indefinitely, starving the pool under load. Added connect (2s) / read (8s) timeouts. The 429 retry/backoff, previously only on the listing, is now a single `withRetry(Supplier<T>)` reused by `fetchByKey` too (the per-ticket hot path, which used to throw a raw 429). A Micrometer `Timer` (`jira.fetch`, p50/p95/p99) makes the "Jira is the bottleneck" claim measurable instead of anecdotal. |
| Jira API token encrypted at rest (opt-in) | `JiraSettingsStore` persisted the token as plaintext JSON on disk. With `JIRA_SETTINGS_SECRET` set, the token is AES-GCM encrypted (key = SHA-256 of the secret) and the file is written owner-only (POSIX 600). Backward compatible: no secret => legacy plaintext, so existing files and the tests keep working. |
| Constant-time API key comparison | `String.equals` returns on the first differing byte, leaking the correct prefix through timing. `ApiKeyAuthFilter` now compares with `MessageDigest.isEqual`. |
| `Standard.investigationSteps` is `LAZY` + `@BatchSize` | It was `EAGER`: every `findAll()` (matcher hot path, per analysis and per gap-report) triggered N+1 secondary selects to load steps the matcher never reads. Now `LAZY` with `@BatchSize(200)`, and the adapter's read methods are `@Transactional(readOnly)` so the mapping still initializes the collection inside the session. N+1 collapses to a couple of queries; the matcher stops paying for data it discards. |
| Matcher pre-tokenizes each Standard once | `TextSimilarity.score(String,String)` re-tokenized the Standard's text for every ticket and the ticket's text for every candidate — in the gap-report that's O(tickets x standards) redundant tokenizations. A `score(Set,Set)` overload plus `CalledStandardMatcher.prepare(...)` tokenize each Standard once and reuse it across all tickets. The greedy algorithm is byte-for-byte identical (verified by 300k fuzz pairs + the existing `TextSimilarityTest`) — a pure speedup, not a behavior change. |
| TF-IDF corpus tokenized once and cached (`DocumentCorpusIndex`) | `DocumentSearchService.search` called `findAllChunks()` (full scan of every chunk of every document) and re-tokenized the whole corpus on every query. A `DocumentCorpusIndex` bean whose `@Cacheable("documentCorpus")` method loads and tokenizes the corpus once, evicted on document upload/delete. Separate bean on purpose: Spring's cache proxy doesn't intercept self-invocation. |
| Bean Validation on request bodies + broader `GlobalExceptionHandler` | Payloads had no size/null guards (a Standard `text` could be arbitrarily large). Added `@NotBlank/@Size/@NotNull` + `@Valid`. The handler, which only mapped `NoSuchElementException`->404 (plus the Jira/document exceptions), now also maps validation->400, upstream Jira failures->502/503 and an unexpected catch-all->500 with a sanitized body (logs the stacktrace, never leaks it). |
| Public docs and actuator info gated by config | Swagger UI and `/v3/api-docs` are toggleable via `SWAGGER_ENABLED`/`API_DOCS_ENABLED` (default on, set false in prod); `management.info.git.mode` dropped from `full` to `simple` so `/actuator/info` stops exposing full commit detail. |
| Dead code removed; `java.time` in the mapper | `CategoryStandard` had zero references; `Requester.requesterBranch` was always `null` (Jira never provides it) and never read — both removed. `JiraCalledMapper` swapped the per-field, non-thread-safe `SimpleDateFormat` for reusable static `DateTimeFormatter`s. |
...
- [x] Performance & resilience pass on the hot paths — Jira adapter timeouts + unified 429 retry + `jira.fetch` metric, read-through Caffeine cache (`openCalleds`/`standards`/`documentCorpus`), `investigationSteps` N+1 fix (LAZY + `@BatchSize`), matcher pre-tokenization, TF-IDF corpus cached.
- [x] Security hardening — constant-time API key comparison, Jira token encrypted at rest (opt-in `JIRA_SETTINGS_SECRET`), Swagger/actuator gated by config, Bean Validation on request bodies, `GlobalExceptionHandler` extended (400/502/503/500 sanitized).
